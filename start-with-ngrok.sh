...
if [ -f "./ngrok" ]; then
    ...
    echo
    echo "🚀 간편 설치: ./install-ngrok.sh 실행"
    echo "   또는"
    echo "📥 수동 설치:"
...
# Linux: gnome-terminal 또는 xterm
if [[ "$OSTYPE" == "darwin"* ]]; then
    # macOS
    osascript -e "tell app \"Terminal\" to do script \"cd $(pwd) && source venv/bin/activate && python run.py\""
else
    # Linux
    if command -v gnome-terminal &> /dev/null; then
        gnome-terminal -- bash -c "cd $(pwd) && source venv/bin/activate && python run.py; exec bash"
    elif command -v xterm &> /dev/null; then
        xterm -hold -e "cd $(pwd) && source venv/bin/activate && python run.py" &
    else
        echo "! 새 터미널을 열 수 없습니다. 백그라운드에서 실행합니다..."
        python run.py &
    fi
fi
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    # macOS
    osascript -e "tell app \"Terminal\" to do script \"cd $(pwd) && $NGROK_CMD http 8000\""
else
    # Linux
    if command -v gnome-terminal &> /dev/null; then
        gnome-terminal -- bash -c "$NGROK_CMD http 8000; exec bash"
    elif command -v xterm &> /dev/null; then
        xterm -hold -e "$NGROK_CMD http 8000" &
    else
        echo "! 새 터미널을 열 수 없습니다. 백그라운드에서 실행합니다..."
        $NGROK_CMD http 8000 &
    fi
